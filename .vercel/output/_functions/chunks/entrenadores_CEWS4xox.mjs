...
async function fetchCoachesDirectly() {
  try {
...
async function fetchCoachStats(coachId) {
  try {
    const { createClient } = await import('@libsql/client');
    const url = undefined                                  ;
    const authToken = undefined                                ;
    ...
    const estadisticas = {};
    const temporadasSet = /* @__PURE__ */ new Set();
    ...
    const estadisticasArray = Object.values(estadisticas).map((season) => {
...
export { fetchCoachStats as a, fetchCoachesDirectly as f };
