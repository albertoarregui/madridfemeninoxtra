...
function getCleanCountryName(country) {
  return country ? country.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/ /g, "_") : "default";
}
async function fetchPlayersDirectly() {
  try {
...
async function fetchPlayerStats(playerId, isGoalkeeper) {
  try {
    const { createClient } = await import('@libsql/client');
    const url = undefined                                  ;
    const authToken = undefined                                ;
...
async function fetchPlayerDebut(playerId) {
  try {
    const { createClient } = await import('@libsql/client');
    const url = undefined                                  ;
    const authToken = undefined                                ;
...
export { fetchPlayerStats as a, fetchPlayerDebut as b, fetchPlayersDirectly as f };
