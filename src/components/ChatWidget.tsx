...
export default function ChatWidget() {
    ...
    const { messages, input, handleInputChange, handleSubmit, status, append, error } = useChat({
        api: '/api/chat',
        onError: (err) => {
    ...
    useEffect(() => {
        console.log('[CHAT] Component Mounted. isOpen:', isOpen);
    }, []);

    useEffect(() => {
        console.log('[CHAT] State changed - isOpen:', isOpen);
        if (isOpen) {
            // Force a small delay to ensure DOM is ready for scrolls if needed
    ...
    return (
        ...
        >
            <div className="pointer-events-auto font-sans relative">
                {!isOpen && (
                    <button
                        ...
                        onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            console.log('[CHAT] Click detected. Previous state:', isOpen);
                            setIsOpen(true);
                        }}
                        className="bg-[#2b2b2b] hover:bg-[#1f1f1f] text-[#ffde59] p-4 rounded-full shadow-2xl border border-white/10 flex items-center gap-2 group transition-transform active:scale-95"
                        style={{ position: 'relative', zIndex: 10 }}
                    >
                        <div className="relative">
                            <Bot className="w-6 h-6 text-[#ffde59]" />
                            <span className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-[#2b2b2b]"></span>
                        </div>
                        <span className="font-bold pr-1 hidden sm:inline text-white">Pregúntame</span>
                        {/* Invisible helper to ensure button has area and doesn't collapse */}
                ...
                {isOpen && (
                    <div
                        className="bg-[#1f1f1f] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] w-[350px] sm:w-[400px] h-[500px] sm:h-[600px] flex flex-col border border-white/10 overflow-hidden"
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[#1f1f1f]">
                            ...
                            {error && (
                                <div className="flex gap-3 items-start">
                                    ...
                                    <div className="bg-red-500/10 border border-red-500/20 text-red-200 rounded-2xl p-3 text-xs leading-relaxed max-w-[80%]">
