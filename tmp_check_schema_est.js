...
async function run() {
    try {
        console.log("Schema for estadisticas_jugadoras:");
        const res1 = await client.execute("SELECT sql FROM sqlite_master WHERE name='estadisticas_jugadoras'");
        console.log(res1.rows[0]?.sql);

        console.log("\nSchema for alineaciones:");
        const res2 = await client.execute("SELECT sql FROM sqlite_master WHERE name='alineaciones'");
