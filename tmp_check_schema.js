...
async function run() {
    try {
        console.log("Schema for goles_rival:");
        const res1 = await client.execute("SELECT sql FROM sqlite_master WHERE name='goles_rival'");
        console.log(res1.rows[0]?.sql);

        console.log("\nSchema for penaltis_fallados:");
        const res2 = await client.execute("SELECT sql FROM sqlite_master WHERE name='penaltis_fallados'");
